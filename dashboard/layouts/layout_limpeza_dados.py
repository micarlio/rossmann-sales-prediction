...
def criar_layout_limpeza_dados(dados): # Refatorar nome da função e parâmetro
    ...
    return html.Div([
        ...
        dbc.Card([
            dbc.CardBody([
                html.H3("Impacto da Limpeza: Distribuição e Uso de Memória", style={'borderLeft': f'5px solid {VERMELHO_ROSSMANN}', 'paddingLeft': '10px'}),
                dbc.Row([
                    dbc.Col(dbc.Card(dbc.CardBody([html.H5("Uso Memória (Bruto)"), html.H4(f"{mem_bruto:.2f} MB")])), md=4),
                    dbc.Col(dbc.Card(dbc.CardBody([html.H5("Uso Memória (Otimizado)"), html.H4(f"{mem_otimizado:.2f} MB")])), md=4),
                    dbc.Col(dbc.Card(dbc.CardBody([html.H5("Redução"), html.H4(f"{red_pct:.1f}%")])), md=4)
                ], className="g-4 mb-4"),
                dcc.Graph(figure=fig_hist_dist)
            ])
        ], className="custom-card mt-4"),
        # Seção de Outliers
        dbc.Card([
